...
describe('Projects', () => {
  ...
  it('can create a project WITH database connection', () => {
    ...
    cy.get('input[placeholder="Enter database URL"]').type(Cypress.env('databaseURL'));
    ...
    // make sure the newly created card has CRAWLING or ACTIVE status
    cy.contains('.ant-card', projectName, { timeout: 10000 }).within(() => {
      cy.get('.ant-tag').should('satisfy', ($el) => {
        const text = $el.text();
        return text.includes('Crawling') || text.includes('Active');
      });
    });
  });
  ...
  after(() => {
    ...
    cy.task('cleanupTestProjects', null, { timeout: 15000 });
  });
});
